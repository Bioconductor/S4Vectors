...
setMethod("merge", c("Vector", "Vector"),
          function (x, y, all = FALSE, all.x = all, all.y = all, sort = TRUE, 
                    suffixes = c(".x", ".y")) 
          {
              stopifnot(isTRUEorFALSE(all.x),
                        isTRUEorFALSE(all.y),
                        isTRUEorFALSE(sort))
              ...
              ans <- x[from(hits)]
              if (all.x) {
                  x_only <- countLnodeHits(hits) == 0L
                  ans <- c(ans, x[x_only])
              }
              if (all.y) {
                  y_only <- countRnodeHits(hits) == 0L
                  ans <- c(ans, y[y_only])
              }
              
              mcols(ans) <- merge(mcols_x, mcols_y, by=hits,
