...
\title{An Overview of the \Rpackage{S4Vectors} package}
...
The \Rpackage{S4Vectors} package provides a framework for representing
...
implemented in the \Rpackage{S4Vectors} package itself e.g. \Rclass{Hits},
\Rclass{Rle}, and \Rclass{DataFrame}). Many more are implemented in the
\Rpackage{IRanges} and \Rpackage{GenomicRanges} infrastructure packages,
...
expect that packages that apply \Rpackage{S4Vectors} to a particular
problem domain will provide vignettes with relevant, realistic examples.

The \Rpackage{S4Vectors} package is available at bioconductor.org and
...
In the context of the \Rpackage{S4Vectors} package, a vector-like object
...
In \Rpackage{S4Vectors} and many other Bioconductor packages, vector-like
...
The \Rclass{Rle} class defined in the \Rpackage{S4Vectors} package is used
...
this, the \Rpackage{S4Vectors} package supports seven additional functions
...
The \Rpackage{S4Vectors} package uses two generic functions, \Rfunction{c}
...
two functions are given the \Rpackage{base} package.
...
To compensate for this, the \Rpackage{S4Vectors} package provides
\Rfunction{aggregate} and \Rfunction{shiftApply} methods
(\Rfunction{shiftApply} is a new generic function defined in
\Rpackage{S4Vectors}) to perform calculations over subsequences
...
In addition, the \Rpackage{S4Vectors} package introduces the
...
See the ``An Overview of the \Rpackage{IRanges} package'' vignette in the
\Rpackage{IRanges} package for many more examples of \Rclass{List} derivatives.
...
The output of \Rfunction{sessionInfo} on the build system after running
this vignette.}
\end{table*}
